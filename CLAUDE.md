...
- **`headless.py`** — `ahl run`'s turn loop: `docker exec -i` of the driver's command per turn with the prompt on stdin and raw output in `turns/<n>/`, `--timeout` per turn, classification (exec exit 125–127, daemon errors and a vanished container are `infra`; otherwise the driver decides), Ctrl-C handling, key usage around each turn, teardown (kill the harness, `chown -R` the writable mounts to the caller, `docker rm -f`), then `trace.json`, `trace.jsonl` and `result.json`. Exit codes follow the run status.
