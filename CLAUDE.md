...
- **`capabilities.py`** — `Capability` value object + `parse_capabilities()` for the `capabilities:` list in `config.yaml` (skill/MCP bundles to preinstall — see `docs/capability-format.md`). Agent-facing and per-harness: wiring differs by harness (native skill-dir discovery vs. fold-in fallback). `render_skill_fallback()` reads a skill bundle's `SKILL.md` for harnesses with no native skill concept.
...
- **`docker.py`** — harness-agnostic Docker plumbing only: `image_name()`, `dockerfile_path()`, `docker_run_args()` (assembles the `docker run` invocation — always `-it`, always mounts workspace at `/workspace`, runs `init-firewall.sh` then either `bash` directly, or `sh -c "<setup_commands> && exec bash"` when there are `packages:` setup commands to run first). Takes two separate volume lists: `extra_volumes` (read-write — harness state the harness itself writes into at runtime, e.g. seeded config/data dirs) and `readonly_volumes` (mounted `:ro` — skill/package capability bundles, so the agent can never write back into host source that's still under active development; verified this nests correctly even when mounted under an already-read-write parent like `/workspace`).
- **`harnesses/`** — one adapter per harness, each implementing the `HarnessAdapter` Protocol (`harnesses/base.py`): `build_env`, `seed`, `wire_capabilities`, `parse_trace`, `start_command`, `start_hints`. Everything specific to a harness — env vars, pre-auth config seeding, capability wiring, native log/session parsing, launch command — lives in that harness's own module (`harnesses/{gemini,opencode,claude,agy}.py`), not spread across shared dispatch tables. `harnesses/base.py` holds only what's genuinely shared (the Protocol, `google_env()` used by both `gemini`/`agy`, the MCP-unsupported warning). `harnesses/__init__.py` is the registry (`get_adapter(name)`) — the single place a new harness gets added. `seed`/`wire_capabilities` write into a per-run subdirectory they own (e.g. `runs/<id>/gemini/`) and return the `(host_path, container_path)` volume mounts for it; `parse_trace` reads that same subdirectory back into a normalized dict, or returns `None` if the harness has no known log/session location yet (`claude`, `agy` — see `TODO`).
- **`cli.py`** — the `up` command orchestrates: load config → create `runs/<timestamp>-<harness>/` → build image → `get_adapter(harness)` → `adapter.seed()` + `adapter.wire_capabilities()` + `wire_packages()` (collecting volume mounts and setup commands) → assemble docker args → run interactively → on exit, `adapter.parse_trace()` and write `trace.json` if not `None`.

**Adding a new harness** means: add `docker/<harness>.Dockerfile`, add it to `SUPPORTED_HARNESSES` in `config.py`, add a new `harnesses/<harness>.py` adapter implementing `HarnessAdapter`, and register it in `harnesses/__init__.py`'s `_ADAPTERS` dict. Every `docker/<harness>.Dockerfile` installs `python3` (apt) and `uv`/`uvx` (copied from the official `ghcr.io/astral-sh/uv` image via a `FROM ... AS uv` stage — `COPY --from=ghcr.io/astral-sh/uv:${UV_VERSION}` directly isn't supported by buildkit) in its runtime stage, so a skill or MCP server bundled as a Python/uv project runs the same way regardless of which harness it's mounted into.
