...
**Live view.** While a turn runs, `ahl run` prints a condensed view of the harness's streamed output to stderr. It shows assistant text (shortened), each tool call with its tool name and a short input, subagent starts, and errors, marked with the agent they come from. A subagent's own messages and tool calls appear only where the harness streams them; OpenCode's stream omits them, and `trace.jsonl` still records them. `--quiet` limits the output to the per-turn summary. The run directory is the same either way.
...
- **AC-11** (unit) Replaying recorded stdout from both harnesses, including a subagent, the live view shows assistant text, tool names, the subagent's start, its tool calls where the harness streams them (Claude Code), and errors, in order. With `--quiet`, only the per-turn summaries appear.
