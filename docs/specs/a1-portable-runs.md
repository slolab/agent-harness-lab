...
AHL becomes usable as a pinned dependency of another repository, such as biotope-bench. Today it assumes the config, `.env` and `runs/` sit in its own checkout, builds images from that checkout, and installs whatever harness version is current at build time. Afterwards the config, env file and run directory can live anywhere, and images build from the installed package. A config can pin its harness version, and every run records which AHL, which image and which harness version produced it, so results can be traced and repeated. Generic config keys cover what callers need next: an optional harness version, data mounts, an existing Docker network, non-secret environment variables and package extras.
...
- An optional harness version in the config; the installed harness version recorded per image; provenance in `session.json`.
- Config keys `mounts`, `network`, `env` and `packages[].extras`.

## Non-goals

- Headless execution (A2) and the Codex harness (A3).
- Pinning uv, base images or system packages.
- Harness versions for gemini, agy and claude-science. DeepSeek keeps its lockfile.
...
- **Build.** `ahl build (--harness NAME | -c CONFIG)` builds `agent-harness-lab:<harness>` from the installed package. It needs no provider key and reads no env file; with `-c` it reads only `harness` and `harness_version`. `ahl up` builds by default in the same way, and `--no-build` skips it.
- **Harness version.** The optional config key `harness_version` (for example `harness_version: 2.1.273`) applies to `claude` and `opencode`: the image installs exactly that version. Without it, the image installs the release current at build time. Setting it for another harness is a config error. No Dockerfile hard-codes a harness version.
- **Image labels.** Every image carries `ahl.harness=<name>`.
- **`session.json`** keeps its fields and gains `ahl: {version, git_sha, git_dirty}` and `image: {name, id, harness_version}`, e.g. `{"name": "agent-harness-lab:opencode", "id": "sha256:…", "harness_version": "1.18.32"}`. `git_sha` and `git_dirty` are null outside a git work tree. `image` describes the image the container started from, also with `--no-build`. `harness_version` is the version installed in the image, pinned or not. It is null only for harnesses whose version AHL does not track: gemini, agy and claude-science.
...
- **AC-4** (unit, docker) No Dockerfile hard-codes a harness version or pins uv, and every image carries `ahl.harness`. With `harness_version` set, the claude and opencode images install exactly that version; without it, the current release (unit: the build receives the version; docker: `claude --version` and `opencode --version` in the images). `harness_version` for another harness is a config error. DeepSeek keeps its lockfile.
- **AC-5** (unit, docker) `session.json` carries the `ahl` and `image` fields, with `image.harness_version: null` for an untracked harness. In the docker tier, `image.id` matches `docker image inspect` and `image.harness_version` matches the version installed in the image, for a pinned and an unpinned build, with and without `--no-build`.
...
- **AC-10** (unit) `config.example.yaml` demonstrates `harness_version`, `mounts`, `network`, `env` and `extras`, and loads through the real config loader with paths resolved against its own directory.

## Freedom to operate

Where the image files live in the package and how they are found; how the harness version reaches the build and is recorded with the image; how config errors are worded; test layout and fixtures.
...
- Without `harness_version`, resolve the current release before building and pass it as a build argument and label, so every image records an exact version that `docker image inspect` can read.
...
- The harness versions installed with and without `harness_version`, how the image records them, and a sample `session.json`.
- The wheel listing showing the image files, the DeepSeek build result, and `uv run pytest -m docker` output.
- `README.md` and `config.example.yaml` document `--env-file`, `--runs-dir`, `ahl build`, `harness_version`, `mounts`, `network`, `env` and `extras`.
