...
- **AC-4** (unit, docker) The Claude Code and OpenCode Dockerfiles default to exact harness versions, and every Dockerfile uses an exact uv version, not `latest` or another moving tag. DeepSeek stays pinned by its lockfile. Every image's labels follow the Interfaces, and `docker image inspect` on the claude and opencode images shows `ahl.harness.version` equal to the installed harness version.
